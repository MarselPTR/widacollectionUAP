        ...
        <div class="max-w-6xl mx-auto px-4 py-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                <p class="text-sm uppercase tracking-[0.4em] text-white/70 wc-reveal" style="--reveal-delay: 40ms;">
                    Detail Produk</p>
                <h1 id="detailTitle" class="text-3xl md:text-4xl font-bold wc-reveal" style="--reveal-delay: 120ms;">
                    Memuat produk...</h1>
                <p id="detailCategory" class="text-white/80 wc-reveal" style="--reveal-delay: 180ms;">Kategori | Tipe
                </p>
            </div>
            <div class="flex flex-wrap gap-3">
                <a href="body"
                    class="inline-flex items-center gap-2 bg-white/15 border border-white/30 px-5 py-2 rounded-full font-semibold hover:bg-white/25 transition wc-reveal"
                    style="--reveal-delay: 240ms;">
                    <i class="fas fa-arrow-left"></i> Kembali ke Katalog
                </a>
                <a id="goToCartBtn" href="cart"
                    class="inline-flex items-center gap-2 bg-white text-primary px-5 py-2 rounded-full font-semibold shadow wc-reveal"
        ...
        <section class="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 md:p-10 grid lg:grid-cols-2 gap-8">
            ...
            <div class="flex flex-col gap-5">
                <div class="flex items-center gap-3 text-sm text-gray-500">
                    <span id="detailType"
                        class="px-3 py-1 rounded-full bg-primary/10 text-primary font-semibold">Fashion</span>
                ...
                <div class="flex flex-wrap gap-4">
                    ...
                    <button id="detailWishlist"
                        class="inline-flex items-center gap-2 px-6 py-3 rounded-full border border-gray-200 text-gray-600 font-semibold hover:border-secondary hover:text-secondary transition">
                        <i class="fas fa-heart"></i>
                        Simpan Wishlist
                    </button>
                    <button id="detailBuyNow"
                        class="inline-flex items-center gap-2 px-6 py-3 rounded-full border border-primary text-primary font-semibold hover:bg-primary/5 transition">
        ...
        <section class="grid lg:grid-cols-3 gap-8">
            ...
            <div class="bg-white rounded-3xl shadow border border-gray-100 p-6 space-y-5">
                ...
                <ul id="highlightList" class="space-y-3 text-gray-600">
                    <li class="flex gap-3"><i class="fas fa-circle-check text-primary mt-1"></i><span>Material tebal &
                            breathable</span></li>
                    <li class="flex gap-3"><i class="fas fa-circle-check text-primary mt-1"></i><span>Standar kurasi
                            Wida Collection</span></li>
                    <li class="flex gap-3"><i class="fas fa-circle-check text-primary mt-1"></i><span>Bonus dust bag
                            eksklusif</span></li>
                </ul>
                <div class="bg-gradient-to-br from-secondary to-primary text-white rounded-2xl p-5 space-y-2">
                    ...
                    <a href="body#live-drop" class="inline-flex items-center gap-2 underline font-semibold">Lihat jadwal
    ...
    <script>
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            async function ensureLogin(action, redirectTo) {
                ...
                const file = window.location.pathname.split('/').pop() || 'product-detail';
            ...
            goToCartBtn?.addEventListener('click', async (e) => {
                e.preventDefault();
                if (!(await ensureLogin('melihat keranjang', 'cart'))) return;
            ...
            const hydrateProduct = (product) => {
                ...
                el('detailBuyNow').onclick = async () => {
                    if (!(await ensureLogin('melanjutkan pembelian', 'checkout'))) return;
                    try {
                        await apiFetchJson('/api/cart/items', {
                            method: 'POST',
                            body: JSON.stringify({
                ...
                try {
                    ...
                    if (!reviews.length) {
                        ...
                        <article class="p-5 rounded-2xl border border-dashed border-gray-200 text-center">
                            ...
                            <a href="orders" class="inline-flex items-center gap-2 px-5 py-2 rounded-full border border-primary text-primary font-semibold hover:bg-primary/5 transition">Buka Halaman Pesanan <i class="fas fa-arrow-right"></i></a>
                        </article>`;
                    } else {
