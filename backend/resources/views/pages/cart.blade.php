...
<body class="font-poppins bg-gray-50 min-h-screen">
  <div class="max-w-6xl mx-auto px-4 py-8 space-y-8">
    <header
      class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between bg-white/80 backdrop-blur rounded-2xl shadow-lg p-6 border border-white/60">
      <div>
        ...
        <p class="text-xs uppercase tracking-[0.4em] text-secondary font-semibold wc-reveal"
          style="--reveal-delay: 90ms;">Langkah 1 dari 3</p>
        <h1 class="text-3xl font-bold text-dark wc-reveal" style="--reveal-delay: 140ms;">Keranjang Belanja</h1>
        <p class="text-gray-500 wc-reveal" style="--reveal-delay: 210ms;">Review ulang item favoritmu sebelum lanjut ke
          pembayaran.</p>
      </div>
      <div class="flex flex-wrap gap-3">
        <a href="body"
          class="inline-flex items-center gap-2 px-5 py-2 rounded-full border border-primary/40 text-primary font-semibold hover:bg-primary/5 transition wc-reveal"
          style="--reveal-delay: 260ms;">
          <i class="fas fa-arrow-left"></i> Lanjut Belanja
        </a>
        <a href="profile"
          class="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-secondary text-white font-semibold shadow-md hover:opacity-90 transition wc-reveal"
      ...
      <div class="lg:col-span-2 space-y-5">
        <div class="flex flex-col gap-4 bg-white rounded-2xl shadow p-5 border border-gray-100">
          ...
          <button type="button" id="clearCart"
            class="self-start inline-flex items-center gap-2 text-sm font-semibold text-primary hover:text-primary/80">
        ...
        <div id="cartEmpty" class="hidden bg-white rounded-2xl p-10 text-center border border-dashed border-primary/40">
          <div
            class="mx-auto h-20 w-20 rounded-full bg-primary/10 flex items-center justify-center text-primary text-3xl">
          ...
          <a href="body#products"
            class="mt-6 inline-flex items-center gap-2 px-6 py-3 rounded-full bg-primary text-white font-semibold shadow hover:bg-primary/90">
      ...
      <aside class="space-y-5">
        <div class="bg-white rounded-2xl shadow border border-gray-100 p-6 space-y-4">
          <div class="flex items-center justify-between">
            ...
            <span class="px-3 py-1 text-xs font-semibold rounded-full bg-secondary/10 text-secondary">Aman &
              transparan</span>
          </div>
          ...
          <div class="border-t border-gray-100 pt-4">
            ...
            <div class="mt-4">
              <label for="shippingSpeed" class="block text-xs uppercase tracking-[0.3em] text-gray-400">Pilih
                Pengiriman</label>
              <select id="shippingSpeed"
                class="mt-2 w-full rounded-xl border border-gray-200 px-4 py-2 focus:outline-none focus:border-primary">
  ...
  <template id="cart-line-template">
    <article class="bg-white rounded-2xl p-5 shadow border border-gray-100 flex flex-col sm:flex-row gap-5">
      <div
        class="w-full sm:w-32 bg-gradient-to-br from-gray-50 to-white rounded-xl flex items-center justify-center overflow-hidden">
      ...
      <div class="flex-1 flex flex-col gap-3">
        <div class="flex items-start justify-between gap-3">
          ...
          <button type="button" class="text-gray-400 hover:text-primary transition" aria-label="Hapus item"
            data-line-remove>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div
            class="inline-flex items-center gap-3 bg-gray-100 rounded-full px-3 py-1 text-sm font-medium text-gray-600">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      const currentRelativeUrl = () => {
        const file = window.location.pathname.split('/').pop() || 'cart';
